...
fn main() {
    ...
    let handle = lib.relocate_with(&[], &[&libc, &libgcc]).unwrap();

    let f = handle.get_sym("c_fun_add_two").unwrap();
    let f: extern "C" fn(i32) -> i32 = unsafe { core::mem::transmute(f) };
    println!("{}", f(2));
    let g = handle.get_sym("c_fun_print_something_else").unwrap();
    let g: extern "C" fn() = unsafe { core::mem::transmute(g) };
    g();
    let f = handle.get_sym("c_test").unwrap();
